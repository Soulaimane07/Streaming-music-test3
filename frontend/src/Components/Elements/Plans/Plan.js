...
function Plan({data, handleCheckout, loading}) {
  ...
  return (
    <div  className=' text-left px-6 relative overflow-hidden  bg-zinc-800 rounded-md pt-10 pb-4 h-fit flex-1 justify-between'>
        ...
        <button onClick={() => handleCheckout(data)} className={`flex justify-center transition-all py-3 mt-14 w-full rounded-full ${bg} hover:bg-opacity-60 font-medium`}> 
          {loading?.subscriptionPlanId === data?.subscriptionPlanId ?
            <Spinner />
          :
            <p> Obtenir Premium {data?.name} </p>
          }
        </button>
    </div>
  )
